// src/edge-functions/googlebot-check.ts
import ipRangeCheck from "ip-range-check";
import {TRUE} from "astro:db";

// Данные IPv4 и IPv6 диапазонов Googlebot
const googlebotRanges = {
    ipv4: [
        "192.178.5.0/27",
        "192.178.6.0/27",
        "192.178.6.128/27",
...
export default async (request, context) => {
    try {
        ...
        const isUAValid = uaPattern.test(userAgent);

        // Проверка IP
        const isIPValid = clientIP !== 'unknown' && (
            ipRangeCheck(clientIP, googlebotRanges.ipv4) ||
            ipRangeCheck(clientIP, googlebotRanges.ipv6)
        );

        // Добавляем заголовки в ответ
        const headers = new Headers({
            'X-Googlebot-IP': clientIP,
            'X-Googlebot-UA': userAgent,
            'X-Is-Googlebot': String(isUAValid && isIPValid)
        });
